...
s1 = cv2.seamlessClone(result, img2, img2_head_mask, center_face2, cv2.MIXED_CLONE,blend=0)

s2 = cv2.seamlessClone(img2,result,img2_head_mask,center_face2,cv2.MONOCHROME_TRANSFER)

s3 = cv2.seamlessClone(img2,result,img2_head_mask,center_face2,cv2.NORMAL_CLONE)

a = 0.7
b = (1.0 - a)
dst = cv2.addWeighted(result, a, img2, b, 0.0)
